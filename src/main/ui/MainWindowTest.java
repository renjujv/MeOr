...
class MainWindowTest extends JFrame{
	...
	MainWindowTest() {
		try {
			setIconImage(Toolkit.getDefaultToolkit().getImage(MainWindowTest.class.getResource(APP_ICON_PATH)));
		} catch (Exception e) {
			System.out.println(e.getMessage());
		}
		setTitle("MeOr - Media Organiser");
		setExtendedState(Frame.MAXIMIZED_BOTH);		//tell me if this code makes the MainWindow start maximized in your system
	...
	private void setMenuBar(){
		...
		JRadioButtonMenuItem themeMenuItem2 = new JRadioButtonMenuItem("Metal");
		themeMenuGroup.add(themeMenuItem2);
		themeMenu.add(themeMenuItem2);

		JRadioButtonMenuItem themeMenuItem3 = new JRadioButtonMenuItem("Windows");
	...
	private void setSearchBar() {
			...
			try {
				searchLabel.setIcon(new ImageIcon(MainWindowTest.class.getResource(SEARCH_LOGO)));
			} catch (Exception e1) {
				e1.printStackTrace();
			}
	...
	private void setCategoryPanel(){
		...
		categoryScrollPane.setPreferredSize(new Dimension(150,getHeight()));
		...
		categoryTree.setBorder(new LineBorder(new Color(0, 0, 0), 1, true));
